                ...
                <div class="d-block mb-4 mb-md-0">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                        <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('siswa-konfirmasi.index') }}">Halaman
                                    Konfirmasi
                                    Siswa</a></li>
                ...
                <div class="card border-0 shadow">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mt-2 mb-1 align-items-center">
                            ...
                            <div class="col-sm-9 fs-6 text-primary">{{ $rule_gaji->lppk }}%</div>
                        </div>
                        <div class="row mt-2 mb-1 align-items-center">
                            <div class="col-sm-3">Percent Unit</div>
                            <div class="col-sm-9 fs-6 text-primary">{{ $rule_gaji->unit }}%</div>
                        </div>
                        ...
                        <div class="row mt-3 mb-1 align-items-center">
                            <div class="col-sm-12"><img src="{{ url('/assets/images/bukti-siswa/'.$data->file) }}"
                                    alt="">
                            </div>
                        </div>
                        ...
                        <form action="{{ route('siswa-konfirmasi.confirm', $data->id) }}" method="post">
                            @csrf
                            @method('put')
                            <button class="btn w-100 btn-success mt-2 confirm"
                        ...
                        <div class="text-center">
                            <form action="{{ route('siswa-konfirmasi.invalid', $data->id) }}" method="post">
                                @csrf
                                @method('put')
                                <textarea class="form-control" name="invalid_message"
                                    placeholder="{{ is_null($data->invalid_message) ? 'Ketik pesan anda...' : $data->invalid_message }}"
                                    rows="4"></textarea>
...
<script>
    $('.confirm').on('click', function (event) {
        ...
        swal({
            ...
            text: "Mengkonfirmasi Tranksaksi " + name,
            icon: 'warning',
            buttons: ["Cancel", "Yes!"],
        }).then((willConfirm) => {
            if (willConfirm) {
